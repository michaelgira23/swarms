/**
 * Fly an autonomous sequence with the Loco Positioning System
 */

const { Crazyradio, utils } = require('../dist/index');

// Whether or not to restart the sequence after complete
const repeatSequence = true;
const sequence = [
	{
		x: 1,
		y: 1,
		...
		do {
			for (const frame of sequence) {
				await drone.commander.setpoint({
					...
					pitch: frame.x,
					yaw: frame.yaw,
					thrust: Math.floor(frame.z * 1000)
				});
				await utils.wait(frame.delayNext || 0);
			}
